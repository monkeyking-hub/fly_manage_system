...
AnimatedInputField::AnimatedInputField(const QString &placeholderText,
                                       bool isPassword,
                                       QWidget *parent)
    : QWidget(parent)
    , label(new QLabel(this))
    , input(new QLineEdit(this))
    , animation(new QPropertyAnimation(label, "pos", this))
{
    ...
    // 设置输入框
    input->setStyleSheet("background: transparent; border: none; border-bottom: 2px solid white; "
...
QString AnimatedInputField::text() const
{
    return input->text();
} // 获取输入框的文本

QLineEdit *AnimatedInputField::lineEdit()
{
    return this->input;
}
