    ...
    , ui(new Ui::adminAddFlightWindow)
{
    ...
    QPixmap scaledPixmap7 = pixmap_company.scaled(ui->img_airlineCompany->size(),
                                                  Qt::KeepAspectRatio,
                                                  Qt::SmoothTransformation);
    QPixmap scaledPixmap8 = pixmap_map.scaled(ui->img_map->size(),
                                              Qt::KeepAspectRatio,
                                              Qt::SmoothTransformation);
...
void adminAddFlightWindow::onAddFlightBtnClicked()
{
    ...
    if (flightNumber.contains(" ") || aircraftModel.contains(" ") || departure.contains(" ")
        || destination.contains(" ") || departure_datestr.contains(" ")
        || arrival_datestr.contains(" ") || company.contains(" ")) {
    ...
    if (flightNumber == "" || aircraftModel == "" || departure == "" || destination == ""
        || departure_datestr == "" || arrival_datestr == "" || company == "") {
        QMessageBox::critical(nullptr,
                              "添加失败",
                              "添加失败: 您需要填写全部信息！",
                              QMessageBox::Ok,
    ...
    emit addFlightRequested(flightNumber,
                            aircraftModel,
                            departure,
                            destination,
                            unixTimeStamp1,
...
void addFlightHandler::handleAddFlight(const QString &flightNumber,
