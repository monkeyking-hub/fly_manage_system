    ...
    , ui(new Ui::adminUpdateFlightWindow)
{
    ...
    QPixmap scaledPixmap1 = pixmap_position.scaled(ui->img_departure->size(),
                                                   Qt::KeepAspectRatio,
                                                   Qt::SmoothTransformation);
    QPixmap scaledPixmap2 = pixmap_position.scaled(ui->img_destination->size(),
                                                   Qt::KeepAspectRatio,
                                                   Qt::SmoothTransformation);
    QPixmap scaledPixmap3 = pixmap_date.scaled(ui->img_date1->size(),
                                               Qt::KeepAspectRatio,
                                               Qt::SmoothTransformation);
    QPixmap scaledPixmap4 = pixmap_date.scaled(ui->img_date2->size(),
                                               Qt::KeepAspectRatio,
                                               Qt::SmoothTransformation);
    QPixmap scaledPixmap5 = pixmap_flight.scaled(ui->img_flightNumber->size(),
                                                 Qt::KeepAspectRatio,
                                                 Qt::SmoothTransformation);
    QPixmap scaledPixmap6 = pixmap_flight.scaled(ui->img_aircraftModel->size(),
                                                 Qt::KeepAspectRatio,
                                                 Qt::SmoothTransformation);
    QPixmap scaledPixmap7 = pixmap_company.scaled(ui->img_airlineCompany->size(),
                                                  Qt::KeepAspectRatio,
                                                  Qt::SmoothTransformation);
    ...
    connect(ui->calendarWidget,
            ...
            this,
            &adminUpdateFlightWindow::selectDate);
    connect(ui->btn_search,
            &QPushButton::clicked,
            this,
            &adminUpdateFlightWindow::onSearchButtonClicked);
    connect(ui->btn_cancel,
            &QPushButton::clicked,
            this,
            &adminUpdateFlightWindow::onCancelButtonClicked);
    connect(ui->btn_save,
            &QPushButton::clicked,
            this,
            &adminUpdateFlightWindow::onSaveButtonClicked);
}
...
void adminUpdateFlightWindow::onSaveButtonClicked()
{
    ...
    //检查是否填了全部内容
    if (flightNumber.contains(" ") || aircraftModel.contains(" ") || departure.contains(" ")
        || destination.contains(" ") || departure_date.contains(" ") || arrival_date.contains(" ")
        || company.contains(" ")) {
        QMessageBox::critical(nullptr,
                              "修改失败",
                              "修改失败: 输入的内容不能含有空格！",
    ...
    //检查是否填了全部内容
    if (flightNumber == "" || aircraftModel == "" || departure == "" || destination == ""
    ...
    connect(reply, &QNetworkReply::finished, [reply, this]() {
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (code == 200) {
                ...
                msgBox.setText("您已成功修改ID为：" + QString::number(this->id)
                               + "的航班信息"); // 设置提示文本
                msgBox.setStandardButtons(QMessageBox::Ok); // 设置标准按钮（此处只有“确定”按钮）
                // 显示消息框
                msgBox.exec();
